      ...
      do {
        ...
        if (parentPtr == superblock.info.rootnode) {
            ...
            //  cout << "---------- CASE: Root is full ----------" << endl;
             // cout << "Root Full Parent: " << parentPtr << endl;
              ...
              //  cout << "Root Node Case Parent: " << parentPtr << endl;
            ...
            //    cout << "Root's Left keys " << parent.info.numkeys << " Right's: " << newRootRight.info.numkeys << endl;
                ...
                parent.Serialize(buffercache, ptr);
                parent.Unserialize(buffercache, ptr);
              } else {
            ...
            // If the root isn't full, just shift over the keys like normal
             ...
             // cout << "---------- CASE: Root is NOT full ----------" << endl;
              ...
              // Increment the number of keys the parent is going to have
              parent.info.numkeys++;
//              cout << "Keyz: " << key.data << " Parentz: " << parentPtr << " numkeys: " << parent.info.numkeys << endl;
        ...
        // If we're just dealing with an interior node, then move the keys over
         ...
         // cout << "---------- CASE: Interior node ----------" << endl;
          ...
          Bubble(parentPtr, middle, rightNode, false);
...
ERROR_T BTreeIndex::Bubble(SIZE_T &n, KEY_T& key, SIZE_T &child, bool goLeft) {
 // cout << "--------------- BUBBLE ---------------" << endl;
 // cout << "Nodezz: " << n << endl;
  ERROR_T rc;
  KEY_T testkey;
  ...
  if (node.info.numkeys >= full) {
    ...
    // cout << "n " << n << endl;
      ...
      if (!goLeft) {
        //Bubble(parent, middle, newParentRightPtr, false);
      }
      else { 
        //Bubble(parent, middle, n, true);
      }

      if (node.info.nodetype == BTREE_ROOT_NODE) {
        BTreeNode rootpointer;
        rootpointer.Unserialize(buffercache, superblock.info.rootnode);

        superblock.info.rootnode = newParentRightPtr;

        rootpointer.Serialize(buffercache, superblock.info.rootnode);
      }
